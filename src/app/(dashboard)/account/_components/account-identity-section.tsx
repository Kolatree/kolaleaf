...
type MeResponse = {
  ...
  // API-007: server renamed the primary email field to `primaryEmail`
  // so the contract reads symmetrically with `secondaryEmails`. Keeps
  // both lists carrying the same `EmailIdentifier` shape under names
  // that telegraph the relationship.
  primaryEmail: {
    id: string;
    email: string;
    verified: boolean;
      ...
      <div className="mt-4 space-y-3">
        ...
        <div>
          ...
          <div className="mt-1 flex items-center gap-2 flex-wrap">
            <span style={{ fontSize: "14px", color: colors.ink }}>
              {me.primaryEmail?.email ?? "—"}
            </span>
            {me.primaryEmail && (
              <VerifiedBadge verified={me.primaryEmail.verified} />
            )}
          </div>
        </div>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="mt-1" style={{ fontSize: "14px", color: colors.ink }}>
        {value}
      </div>
    </div>
  );
...
function ChangePasswordForm({ onDone }: { onDone: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      const data = await res
        .json()
        .catch(() => ({ error: "Unable to change password" }));
      if (res.status === 401) {
  ...
  return (
    <div
      ...
      style={{ background: colors.pageBg, borderRadius: "10px" }}
    >
      <div>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" style={secondaryButton} onClick={onDone}>
          Cancel
        </button>
      </div>
    </div>
...
function ChangeEmailForm({ onDone }: { onDone: () => void }) {
  ...
  if (sent) {
    return (
      ...
      >
        We&apos;ve sent a verification link to <strong>{sent}</strong>. Click it
        within 24 hours to finish.
        <div className="mt-2">
          <button type="button" style={secondaryButton} onClick={onDone}>
            Close
          </button>
    ...
    >
      ...
      <div className="flex gap-2">
        <button
          type="button"
          style={primaryButton}
          disabled={busy || !current || !newEmail}
          onClick={submit}
        >
