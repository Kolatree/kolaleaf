...
export type TransactionClient = {
  ...
  transferEvent: Pick<typeof prisma.transferEvent, 'create'>
}
...
interface TransitionParams {
  ...
  /** When provided, runs inside the caller's transaction instead of creating a new one. */
  tx?: TransactionClient
}

export async function transitionTransfer(params: TransitionParams): Promise<Transfer> {
  const { transferId, toStatus, actor, actorId, metadata, expectedStatus, tx } = params

  if (tx) {
    return executeTransition(tx, { transferId, toStatus, actor, actorId, metadata, expectedStatus })
  }

  return prisma.$transaction(async (txClient) => {
    return executeTransition(txClient, { transferId, toStatus, actor, actorId, metadata, expectedStatus })
  })
}

async function executeTransition(
  tx: TransactionClient,
  params: Omit<TransitionParams, 'tx'>,
): Promise<Transfer> {
