...
describe("issuePendingEmailCode", () => {
  ...
  it("upserts a fresh row with attempts=0, clears verifiedAt, sends email", async () => {
    ...
    expect(call.where.kind_identifier.kind).toBe("EMAIL");
    expect(call.where.kind_identifier.identifier).toBe("a@b.com");
    expect(call.create.kind).toBe("EMAIL");
    expect(call.create.identifier).toBe("a@b.com");
...
describe("verifyPendingEmailCode", () => {
  const future = () =>
    new Date(Date.now() + PENDING_CODE_TTL_MINUTES * 60 * 1000);
  ...
  it("returns no_token when nothing found", async () => {
    mockFindUnique.mockResolvedValueOnce(null);
    const out = await verifyPendingEmailCode({
      email: "a@b.com",
      code: "123456",
    });
  ...
  it("returns expired when expiresAt is past and not yet verified", async () => {
    mockFindUnique.mockResolvedValueOnce(
      baseRow({ expiresAt: past() }) as never,
    );
    const out = await verifyPendingEmailCode({
      email: "a@b.com",
      code: "123456",
