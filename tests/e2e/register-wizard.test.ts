...
vi.mock("@/lib/auth/tokens", async () => {
  const actual =
    await vi.importActual<typeof import("@/lib/auth/tokens")>(
      "@/lib/auth/tokens",
    );
  return {
    ...actual,
...
describe("Register wizard E2E", () => {
  it("send-code → verify-code → complete-registration produces a verified user + session", async () => {
    ...
    const pending = await prisma.pendingVerification.findUnique({
      where: {
        kind_identifier: { kind: "EMAIL", identifier: email.toLowerCase() },
      },
    });
    ...
    // Step 2: verify-code
    const r2 = await verifyCode(
      req("/api/v1/auth/verify-code", { email, code }),
    );
    ...
    const pendingVerified = await prisma.pendingVerification.findUnique({
      where: {
        kind_identifier: { kind: "EMAIL", identifier: email.toLowerCase() },
      },
    });
    expect(pendingVerified!.verifiedAt).not.toBeNull();
    expect(pendingVerified!.claimExpiresAt!.getTime()).toBeGreaterThan(
      Date.now(),
    );
    ...
    const pendingAfter = await prisma.pendingVerification.findUnique({
      where: {
        kind_identifier: { kind: "EMAIL", identifier: email.toLowerCase() },
      },
    });
