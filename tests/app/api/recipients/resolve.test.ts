...
import { POST } from '@/app/api/recipients/resolve/route'
...
function makeRequest(body: unknown, opts: { userId?: string } = {}): Request {
  return new Request('http://localhost/api/recipients/resolve', {
    method: 'POST',
    headers: {
...
describe('POST /api/recipients/resolve', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('returns 400 on invalid JSON', async () => {
    const res = await POST(
      new Request('http://localhost/api/recipients/resolve', {
        method: 'POST',
