# Review Request -- Wave 2a Phase 12 Shake-to-Report Slice

**Ready for Review:** YES for local shake-to-report support drafting. This does not claim native Sentry SDK crash capture.
**Date:** 2026-05-19
**Branch / worktree:** `main` in `/Users/ao/Documents/projects/Kolaleaf`

## Summary

Adds a global iOS shake-to-report flow. A shake gesture opens a user-confirmed support sheet, then an editable `mailto:` draft to `support@kolaleaf.com`. The generated draft includes only low-risk diagnostics: source, app version/build, OS, device model, and timestamp.

No screenshots, logs, account details, transfer IDs, phone numbers, email addresses, cookies, or tokens are collected or attached automatically. This keeps the Phase 12 beta-feedback affordance aligned with Kolaleaf's remittance privacy posture without adding a new SDK/provider dependency.

## Files Changed In This Slice

- `ios/Kolaleaf/Features/Feedback/{FeedbackDraft,FeedbackReportView,ShakeToReport}.swift` — draft model, support sheet, and UIKit shake responder bridge.
- `ios/Kolaleaf/App/KolaleafApp.swift` — global `.shakeToReport(...)` modifier and `.sheet(item:)` feedback presentation.
- `ios/KolaleafTests/Features/Feedback/FeedbackDraftTests.swift` — guards privacy-preserving draft body and encoded support mail URL.
- `ios/Kolaleaf.xcodeproj/project.pbxproj` — regenerated by XcodeGen.
- `handoff/BUILD-LOG.md` and `handoff/REVIEW-REQUEST.md` — current status and validation evidence.

## Validation

- `xcodegen generate` — regenerated `ios/Kolaleaf.xcodeproj`.
- XcodeBuildMCP `test_sim` with `-only-testing:KolaleafTests/FeedbackDraftTests -only-testing:KolaleafTests/AppLocaleTests -only-testing:KolaleafTests/LocalizationCatalogTests` — 8 tests passed.
...
- Native iOS Sentry SDK crash capture is still a separate product/ops decision because it needs SDK dependency, DSN, release tagging, and dSYM upload policy.
