...
vi.mock("@/lib/auth/middleware", () => ({
  ...
  setPendingTwoFactorCookie: vi.fn(
    () => "kolaleaf_pending_2fa=challenge; HttpOnly",
  ),
...
describe("POST /api/v1/auth/login", () => {
  ...
  it("returns 422 for legacy bare-string identifier (Step 21 contract)", async () => {
    const res = await POST(
      makeRequest({ identifier: "a@b.com", password: "x" }),
    );
    expect(res.status).toBe(422);
  });
  ...
  it("returns 401 with reason=invalid_credentials for invalid credentials", async () => {
    ...
    // dispatch can distinguish bad creds from internal errors (split below).
    expect(json.reason).toBe("invalid_credentials");
    expect(typeof json.error).toBe("string");
    expect(mockRecordFailure).toHaveBeenCalledWith("a@b.com", undefined);
  });

  it("returns 500 with reason=internal_error when an unexpected error fires", async () => {
    // Phase 1 review fix: a DB outage or any non-credential exception used to
    // surface as a 401 'Login failed' to clients, masquerading as bad creds.
    // Now it correctly returns 500 with a distinct reason and no false
    // 'Email or password incorrect' message reaches the user.
    mockLogin.mockRejectedValue(new Error("ECONNREFUSED postgres:5432"));

    const res = await POST(
      makeRequest(validBody("a@b.com", "Right-Password-1234")),
    );
  ...
  it("returns 429 with canonical ErrorEnvelope when login rate limit is exceeded", async () => {
    ...
    const json = await res.json();
    // Phase 1 review fix: { error, reason } envelope; retry-after travels via
    // the Retry-After header, not the body.
